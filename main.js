...
const topBackground = Array.from(
  document.querySelectorAll(".header-img-wraper img")
);
...
function changeBackground(background) {
  ...
  for (let j = 0; j < background.length; j++) {
    background[j].style.cssText =
      "transition: transform 7s ease-in-out, opacity 0.5s";
  }
...
const indexOfAbility = Array.from(
  document.querySelectorAll(".index-of-ability")
);
const abilityRange = Array.from(
  document.querySelectorAll(".ability-range-counter")
);
...
window.addEventListener("scroll", function runSkillBar() {
  if (
    window.pageYOffset >
    skillContainer.offsetTop - window.outerHeight + 300
  ) {
    showSkill();
...
////to the top button
const goUpButton = document.querySelector(".go-to-top");
